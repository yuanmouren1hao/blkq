...
define(['underscore', './util', './RelationshipManager', './Table'], function (_, util, RelationshipManager, Table) {
	...
	setSharedStringCollection: function (stringCollection) {
        ...
        */
		...
		exportMerge: function (doc,value) {
            var mergecell = doc.createElement('mergeCell');
			mergecell.setAttribute('ref',value);
            return mergecell;
        },
        ...
        toXML: function () {
            ...
            for(var row = 0, l = data.length; row < l; row++) {
                ...
                for(var c = 1; c < cellCount; c++) {
                    ...
                    if (typeof dataRow[c] == 'string') {                  	
                        cellValue =dataRow[c];
                    }
                    
                    var cell, metadata = dataRow[c] || {};
			...
			if(this._merges.length>0){
				var merge=doc.createElement('mergeCells');
				merge.setAttribute('count',this._merges.length);
				for(var i=0;i<this._merges.length;i++){
					merge.appendChild(this.exportMerge(doc,this._merges[i]));
				}
				worksheet.appendChild(merge);
			}
        ...
        exportColumns: function (doc) {
            ...
            for(var i = 2, l = this.columns.length; i < l; i++) {
