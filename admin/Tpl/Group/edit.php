...
<body>
	<form action="{:U('group/edit')}" method="post" name="form" id="form">
				...
				<div class="input-label">
					...
					<div class="label-box">
						...
						<volist name="appinfo" id="vo">
							<div class="app" appid="{$vo.tbid}">
								<img src="../../oa/{$vo.icon}" alt="{$vo.name}" title="{$vo.name}">
								<span class="del">删</span>
							</div>
						</volist>
						</div>
						<a class="btn btn-mini" href="javascript:;" menu="addapps">添加应用</a>
						<input type="hidden" name="val_apps_id" id="val_apps_id" value="{$applist}">
					</div>
				</div>
			</div>
		</div>
		<div class="bottom-bar">
			<div class="con">
				<a class="btn btn-large btn-primary fr" menu="submit"
					...
					class="btn btn-large" menu="back" href="index.html"><i
					class="icon-arrow-left"></i> 返回分组列表</a>
			</div>
		</div>
	</form>
