					...
					<volist name="list_d" id="vo">
						<li class="padding-big-bottom">
							<div class="media media-x">
								<a class="float-left" href="<?php echo U("Doctor/doctor_detail");?>?id={$vo.id}">
									<img src="{$vo.image}" width="150px" class="radius" alt="{$vo.name}">
								</a>
								<div class="media-body padding-left padding-right padding-top">
									<div><a href="<?php echo U("Doctor/doctor_detail");?>?id={$vo.id}"><strong>{$vo.name}</strong></a></div>
									<div>{$vo.desc}</div>
								</div>
							</div>
						</li>
